...
> **Decisions taken with the solicitor (2026-06-28) — see `../SOLICITOR-DECISIONS-2026-06-28.md`.** Now reflected in the drafts: lawful-basis **model wording** included; **Nicola Youngman = Data Protection Lead** (no statutory DPO at pilot scale, conflict-of-interest documented); **Children's Code = principles-only** + DPIA; lock-screen push setting **kept (off by default)**; Resend login emails **kept with controls**; and the **welfare-flag retention rule = 365 days then delete** (export/mark-recorded + minimal no-PII audit) — replacing the old indefinite-retention wording. ⚙ The welfare-retention **code change** (reversing the current purge protection) is being implemented separately.
